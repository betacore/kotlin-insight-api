    ...
    private suspend fun <T : InsightEntity> parseObject(
        ...
        val result = kobj.primaryConstructor
            ...
            ?.map { parameter ->
                ...
                val result = when {
                    definedClass == Int::class.java -> (value as String?)?.toInt()
                    definedClass == Float::class.java -> (value as String?)?.toFloat()
                    definedClass == Double::class.java -> (value as String?)?.toDouble()
                    definedClass == Boolean::class.java -> (value as String?)?.toBoolean()
